...
sexGap %>% filter(Year >= 1908 & Year <= 1924) %>% ggplot(aes(x = Year, y = FemaleAndMaleDifference)) +
  geom_bar(stat = "identity", fill = "skyblue4") +
  labs(title = "Ratio Comparison From Female to Male Athletes (1908-1924)", y = "Female to Male Athlete Ratio") +
  geom_text(aes(label = Year), vjust = -0.25)

sexGap %>% filter(Year >= 1932 & Year <= 1952) %>% ggplot(aes(x = Year, y = FemaleAndMaleDifference)) +
  geom_bar(stat = "identity", fill = "skyblue4") +
  labs(title = "Ratio Comparison From Female to Male Athletes (1932-1952)", y = "Female to Male Athlete Ratio") +
  geom_text(aes(label = Year), vjust = -0.25)

sexGap %>% filter(Year >= 1984 & Year <= 2000) %>% ggplot(aes(x = Year, y = FemaleAndMaleDifference)) +
  geom_bar(stat = "identity", fill = "skyblue4") +
  labs(title = "Ratio Comparison From Female to Male Athletes (1984-2000)", y = "Female to Male Athlete Ratio") +
  geom_text(aes(label = Year), vjust = -0.25)
```

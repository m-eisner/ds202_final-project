...
## Exploration and Vizualization through Questions Raised

Here, we will dive into exploring, visualizing, and answering the questions proposed in this project.

**1. How are medals distributed among countries over the history of the games?**

In order to effectively determine the medal distribution trends for different countries, the data must first be cleaned. There are several countries that have multiple versions (appended number or old name), so these need to be combined into single instances. We are alos only interested in the Country, Year, Season, Medal and City columns as these are the only metrics that relate to the country and the medal count. Finally, we use the World map data which contains country names along with their coordinates for mapping. Since the Olympics dataset has lots of team names that are not country names (boat names, independent athletes, clubs, etc.), so when it is joined with the world data, it filters out any team that does not represent a country.
```{r}
...
There are quite a few events that were removed over the years but there are also events that have been removed and reintroduced. Some of the discontinued events include Art Competitions (1912-1948), Men's Tug-Of-War(1900-1920), Men's Military Ski Patrol.
...
## Conclusion
The Olympics provides a very rich pool of data for us to work with. As shown by our various analyses, there is much to be explored in this dataset as evidenced by the variety of questions proposed. To highlight some of our findings, some countries, like the US, are very consistent performers in the games. Other countries like Germany and China, tend to be inconsistent for a myriad of reasons. Additionally, countries tend to perform better when hosting the games. It also seems that as time has progressed, the gender gap in participation has shrunk considerably. We also saw that those who weight closer to 75 kg won more medals than those who were significantly above or under a weight of 75 kg. The summer games have had a history higher attendance than the winter games. It is also noteworthy that only five members of the Individual Olympic Athletes team have been able to win a medal of any kind. There are also some strange events that have been removed, like the Tug-Of-War in 1920. Of all Olympians, it was seen that one stands out in sheer medal count, and his name is Michael Phelps. As a final note, it was seen that GDP is not a strong indicator of how strong a country will perform in the Olympics.

## Personal Contributions
Matthew Eisner - Responsible for Questions 11 and 12, wrote the Introduction, Data Aquisition, and Conclusion sections. 

Jeffrey Kinard - Responsible for Questions 1 and 2, formatted and helped fix errors in the report

Patrick Origer - Responsible for Questions 3 and 4.

Zachary Johnson - Responsible for Questions 5 and 6.

Muhammad Idris - Responsible for Questions 9 and 10.

Cody Nielsen - Responsible for Questions 7 and 8.

## References
120 Years of Olympic History. (n.d.). Kaggle: Your Machine Learning and Data Science Community. https://www.kaggle.com/heesoo37/120-years-of-olympic-history-athletes-and-results?select=athlete_events.csv

Ancient Olympic Games. (n.d.). The International Olympic Committee. https://www.olympic.org/ancient-olympic-games
